...
def solve_sudoku(board, solutions, row=0, col=0):
    if row == 9:
    ...
    if board[row][col] != '':
        solve_sudoku(board, solutions, next_row, next_col)
    else:
...
def solve(request):
    ...
    return render(request, 'result.html', {'input_board': board, 'solutions': solutions})
